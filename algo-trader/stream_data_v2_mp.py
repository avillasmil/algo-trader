...
class SymbolProcessor:
    ...
    def process_bar(self, bar, current_time):
...
async def bar_data_handler(bar):
    ...
    if bar.symbol in symbol_processors:
        # Submit the symbol's processing to the process pool for parallel execution
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            process_pool, symbol_processors[bar.symbol].process_bar, bar, current_time
        )
...
async def run_stream():
    try:
        ...
        process_pool.shutdown()  # Ensure the process pool is shut down
